# Kartodromo di Casaluce - Mod Track per Assetto Corsa

**v1.0.0**

Circuito kart **Kartodromo di Casaluce** (CE), Campania.
Tracciato tecnico su ~1000 metri, larghezza 8m, con layout **default (CW)** e **reverse (CCW)**.
Pista asfaltata con illuminazione notturna, adatta a kart, minimoto, supermotard e pitbike.
...
- **Install** — Installa la mod e i componenti aggiuntivi in Assetto Corsa, senza dipendenze bash.
  ...
  - **Kartodromo di Casaluce** — copia la cartella mod in `content/tracks/`
...
Il file Blender **`casaluce.blend`** e' il **sorgente principale unico** della pista.
...
- Lo script `create_reverse_blend.py` apre `casaluce.blend`, ruota tutti gli empty `AC_*`
  di 180° sull'asse Z (per invertire la direzione di partenza), e salva come `casaluce_reverse.blend`.
...
**Non serve mai modificare `casaluce_reverse.blend` manualmente.**
...
```bash
# Step 1 - Export KN5
blender --background casaluce.blend --python scripts/export_kn5.py

# Step 2 - Setup mod folder
.venv/bin/python scripts/setup_mod_folder.py

# Step 3 - AI line CW
blender --background casaluce.blend --python scripts/generate_ai_line.py

# Step 4 - Reverse blend
blender --background casaluce.blend --python scripts/create_reverse_blend.py

# Step 5 - Export KN5 reverse
TRACK_REVERSE=1 blender --background casaluce_reverse.blend --python scripts/export_kn5.py

# Step 6 - AI line CCW
TRACK_REVERSE=1 blender --background casaluce_reverse.blend --python scripts/generate_ai_line.py

# Copy KN5 files to mod
cp casaluce.kn5 mod/casaluce/casaluce.kn5
cp casaluce_reverse.kn5 mod/casaluce/casaluce_reverse.kn5
```
...
- **Kartodromo di Casaluce** — senso orario (CW)
- **Kartodromo di Casaluce [reverse]** — senso antiorario (CCW)
...
- **Kartodromo di Casaluce** — il circuito (copiato in `content/tracks/casaluce/`)
...
2. Selezionare "Kartodromo di Casaluce" dal menu
3. Verificare:
...
4. Selezionare il layout "Kartodromo di Casaluce [reverse]" e verificare che l'auto parta in senso antiorario
...
```bash
source .venv/bin/activate
python tools/track_viewer.py mod/casaluce/casaluce.kn5
```
**Windows:**
```cmd
.venv\Scripts\activate
python tools\track_viewer.py mod\casaluce\casaluce.kn5
```
...
```
casaluce-track/
├── casaluce.blend                  # Sorgente Blender MASTER (geometria, materiali, empty)
├── casaluce_reverse.blend          # Generato automaticamente (non modificare!)
├── casaluce.kn5                    # Modello 3D formato AC (generato)
├── casaluce_reverse.kn5            # Modello 3D reverse (generato)
├── centerline.json                 # Punti centerline del tracciato (81 punti, CW)
...
├── layout.svg                      # Versione vettoriale del tracciato
├── cover.png                       # Immagine copertina (preview + outline in AC)
├── scripts/
│   ├── create_track.py             # Genera casaluce.blend da centerline.json
...
└── mod/casaluce/                   # Mod finale pronta per AC
    ├── casaluce.kn5                # KN5 layout default (CW)
    ├── casaluce_reverse.kn5        # KN5 layout reverse (CCW)
    ├── models_default.ini          # -> casaluce_reverse.kn5
    ├── models_reverse.ini          # -> casaluce.kn5
...
| Cordoli | 1.0 m x 8 cm (12 segmenti nelle curve) |
| Erba laterale | 2 m per lato |
| Muri | 1.5 m altezza, 1.0 m spessore |
| Griglia partenza | 5 posizioni |
| Pit boxes | 5 |
| Direzione default | Orario (CW) |
| Direzione reverse | Antiorario (CCW) |
| Coordinate GPS | 41.0072, 14.1892 |
| Formato KN5 | v6, exporter custom Python |
| Texture | PNG embedded (D3DX11 auto-detect) |
| Record | 0:37.974 (Gianluigi) |
| Illuminazione | Notturna disponibile |
...
1. `create_reverse_blend.py` apre `casaluce.blend`
2. Per ogni empty con nome che inizia con `AC_`, aggiunge `pi` (180°) alla rotazione Z
3. Salva come `casaluce_reverse.blend`
...
- **Layout reverse non appare in AC:** Verificare che esistano `models_reverse.ini` nella root (punta a `casaluce_reverse.kn5`), `ui/reverse/ui_track.json`, e `reverse/ai/fast_lane.ai`. Pulire la cache AC e CM.
