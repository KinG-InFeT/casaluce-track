#!/usr/bin/env python3
"""Cross-platform CLI build for Casaluce mod.
...
def main():
    blend_file = os.path.join(ROOT_DIR, "casaluce.blend")
    if not os.path.isfile(blend_file):
        print("Error: casaluce.blend not found.")
    ...
    reverse_blend = os.path.join(ROOT_DIR, "casaluce_reverse.blend")
    ...
    mod_dir = os.path.join(ROOT_DIR, "mod", "casaluce")
    ...
    zip_path = os.path.join(builds_dir, "casaluce.zip")
        ...
        with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
            for dirpath, dirnames, filenames in os.walk(mod_dir):
                for fn in filenames:
                    ...
                    arcname = os.path.join("casaluce", os.path.relpath(full, mod_dir))
                    zf.write(full, arcname)
        size_kb = os.path.getsize(zip_path) / 1024
        print(f"  builds/casaluce.zip ({size_kb:.0f} KB)")
    ...
    print("To share:   send builds/casaluce.zip")
