#!/usr/bin/env python3
"""Kartodromo di Casaluce Track Manager - PyQt5 pipeline manager with 3D preview."""
...
DEFAULT_KN5 = os.path.join(ROOT_DIR, "mod", "casaluce", "casaluce.kn5")
...
_BLEND = os.path.join(ROOT_DIR, "casaluce.blend")
_BLEND_REV = os.path.join(ROOT_DIR, "casaluce_reverse.blend")
...
class BuildTab(QWidget):
    ...
    def _on_all_done(self):
        ...
        for kn5_name in ["casaluce.kn5", "casaluce_reverse.kn5"]:
...
class InstallWorker(QThread):
    ...
    def _install_track(self):
        ...
        mod_src = os.path.join(ROOT_DIR, "mod", "casaluce")
        kn5_file = os.path.join(mod_src, "casaluce.kn5")
        if not os.path.isdir(mod_src):
            self._log("Errore: cartella mod/casaluce non trovata. Esegui prima la build.", "#ff5555")
            return
        if not os.path.isfile(kn5_file):
            self._log("Errore: casaluce.kn5 non trovato nella cartella mod.", "#ff5555")
            return
        dest = os.path.join(self.ac_dir, "content", "tracks", "casaluce")
        if os.path.isdir(dest):
            shutil.rmtree(dest)
        shutil.copytree(mod_src, dest)
        self._log("[PISTA] Kartodromo di Casaluce installata!", "#55cc55")
    ...
    def _clean_caches(self):
        ...
        if os.path.isdir(ac_cache):
            for subdir in ("ai_grids", "ai_payloads"):
                pattern = os.path.join(ac_cache, subdir, "casaluce__*")
...
class InstallTab(QWidget):
    def __init__(self, parent=None):
        ...
        self.chk_track = QCheckBox("Kartodromo di Casaluce")
...
INFO_FIELDS = [
    ("name",      "Name",      "line",  "Kartodromo di Casaluce"),
    ("city",      "City",      "line",  "Casaluce"),
...
class TrackManager(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Kartodromo di Casaluce - Track Manager")
    ...
    def _auto_export_kn5(self):
        blend = os.path.join(ROOT_DIR, "casaluce.blend")
