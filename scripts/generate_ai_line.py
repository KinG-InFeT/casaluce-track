...
Run with: blender --background casaluce.blend --python scripts/generate_ai_line.py
...
def extract_centerline():
    ...
    # So: CW in Blender → CCW in AC, and CCW in Blender → CW in AC.
    # Default layout = CW in AC → need CCW in Blender.
    # Reverse layout = CCW in AC → need CW in Blender.
    if _REVERSE:
        # CCW in AC → need CW in Blender
        if signed_area > 0:  # currently CCW in Blender
            centerline = centerline[::-1]
            print(f"  Reversed to CW in Blender (→ CCW in AC)")
    else:
        # CW in AC → need CCW in Blender
