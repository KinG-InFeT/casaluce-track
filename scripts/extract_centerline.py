...
scales to the real-world track length (~1000 m), and saves to centerline.json.
...
TARGET_LENGTH = 1000.0  # meters (will be overridden by config if present)
    ...
    while i < len(tokens):
        ...
        if cmd == 'M':
            ...
            while i + 5 < len(tokens) and tokens[i] not in 'MmCcLlHhVvZzSsQqTtAa':
                ...
                p2 = (x2, y2)
                p3 = (x3, y3)
                # Sample 10 points per Bezier segment
...
def compute_centerline(outer, inner, n_samples=500):
    ...
    # Resample both contours to many points
...
def main():
    print("=" * 60)
    print("Casaluce Track — Centerline Extractor")
    print("=" * 60)

    # Load config for target length
    ...
    # Find the black path (#080808) - it's the track ribbon
    # Extract the d attribute from the black path
    pattern = r'<path d="([^"]+)"[^>]*fill="#080808"'
    match = re.search(pattern, svg)
    if not match:
        # Try alternate: the third path element (index 2)
    ...
    # Ensure CW direction (negative signed area)
    area = signed_area(cl)
    ...
    # Flip Y axis (SVG Y goes down, Blender Y goes up)
    ...
    # Save
    print(f"\nSaving {len(result)} control points to {OUTPUT_PATH}...")
