#!/bin/bash
# Casaluce - Installazione/Aggiornamento mod per Assetto Corsa
...
echo "=== Casaluce - Installazione mod ==="
echo "AC trovato in: $AC_DIR"
echo ""

# --- 1. Installazione pista Casaluce ---
MOD_NAME="casaluce"
...
echo "[PISTA] Installando Casaluce..."
if [ -d "$DEST" ]; then
    rm -rf "$DEST"
fi
cp -r "$MOD_FULL" "$DEST"
echo "[PISTA] Casaluce installata!"

# Clean AC engine cache for this track (AI grids, payloads, mesh metadata)
AC_CACHE="$AC_DIR/cache"
if [ -d "$AC_CACHE" ]; then
    cleaned=0
    for f in "$AC_CACHE"/ai_grids/casaluce__* "$AC_CACHE"/ai_payloads/casaluce__*; do
        if [ -f "$f" ]; then
            rm -f "$f"
            cleaned=$((cleaned + 1))
        fi
...
echo ""
echo "Pista:"
if [ -d "$TRACKS_DIR/casaluce" ]; then
    kn5=$(find "$TRACKS_DIR/casaluce" -name "*.kn5" -maxdepth 1 | head -1)
    if [ -n "$kn5" ]; then
        size=$(du -h "$kn5" | cut -f1)
        echo "  [OK] Casaluce ($size)"
    else
        echo "  [!!] Casaluce (kn5 mancante!)"
    fi
else
    echo "  [--] Casaluce (non installata)"
fi
echo ""
...
echo "Seleziona 'Casaluce' dal menu piste."
